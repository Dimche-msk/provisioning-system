...
### Настройка моделей (Model Configuration)

Файлы моделей описывают физические характеристики конкретных устройств (количество кнопок, линий, поддержку панелей расширения). Они находятся в папке `models/` внутри вендора.

#### Основные параметры модели:
*   **`id`**: Уникальный ID модели (например, `yealink-T46U`).
*   **`name`**: Отображаемое имя модели.
*   **`type`**: Тип устройства:
    *   `phone`: Обычный IP-телефон.
    *   `gateway`: Многопортовый шлюз (FXS/FXO) или АТС.
    *   `expansion-module`: Панель расширения (sidecar).
*   **`fake_deploy`**: Параметр, который должен быть true если это не простой шлюз, а описание абонентов АТС 
*   **`max_account_lines`**: Максимальное количество SIP-аккаунтов или линий.
    *   *Для шлюзов* этот параметр определяет **количество физических портов**. Система будет позволять настроить ровно столько аккаунтов, сколько указано здесь.
*   **`maximum_expansion_modules`**: Максимальное количество подключаемых панелей расширения.
*   **`supported_expansion_modules`**: Список ID моделей панелей расширения, которые можно подключить к этому телефону.
*   **`own_hard_keys`**: Количество программируемых кнопок на самом устройстве.

#### Описание кнопок (`keys`):
Секция `keys` описывает физические кнопки устройства для отрисовки в UI и маппинга на параметры конфигурации.
*   **`index`**: Номер кнопки.
*   **`type`**: Тип кнопки по умолчанию (`line`, `memory` и т.д.).
*   **`label`**: Текст подписи в интерфейсе.
*   **`settings`**: Карта сопоставления абстрактных полей (value, label, type) с конкретными тегами вендора.
    ```yaml
    settings:
      ...
      type: "linekey.1.type"
    ```
    Это позволяет использовать универсальные шаблоны в `features.yaml`, которые будут подставлять правильные теги для разных моделей одного вендора.

### Настройка шлюзов и многопортовых устройств
При настройке шлюзов (например, Yeastar TA3200) используйте `type: gateway` и установите `max_account_lines` равным количеству портов. В основном шаблоне (`phone.tpl`) такие устройства обычно настраиваются через цикл по всем линиям:
```xml
{% for line in account.lines %}
  <!-- Настройка порта {{ line.number }} -->
  <Port{{ line.number }}>
