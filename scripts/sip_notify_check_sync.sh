...
# Проверка аргументов
if [ "$#" -lt 2 ]; then
    echo "Usage: $0 <target_ip> <my_ip> [target_port] [target_username]"
    exit 1
fi
...
TARGET_USER="${4:-check-sync}" # Имя пользователя (extension) по умолчанию check-sync
...
printf -v FORMATTED_PACKET "%b" "NOTIFY sip:$TARGET_USER@$TARGET_IP:$TARGET_PORT SIP/2.0\r\n\
Via: SIP/2.0/UDP $MY_IP:5060;branch=$BRANCH\r\n\
From: <sip:provisioning@$MY_IP>;tag=$FROM_TAG\r\n\
To: <sip:$TARGET_USER@$TARGET_IP>\r\n\
Call-ID: $CALL_ID\r\n\
CSeq: 1 NOTIFY\r\n\
